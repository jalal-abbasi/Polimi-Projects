...
theta =  atan (data.vel ./ ( Rb0 + data.pos ));

%% Curvature radius of the pitch profile

rho0 = ( data.vel.^2 + (Rb0+data.pos).^2).^(1.5) ./ ( (Rb0+data.pos).^2 - (Rb0+data.pos).*data.acc + 2*data.vel.^2 ) ;
...
r = sqrt( Rr^2 + (Rb0+data.pos).^2 - 2*Rr*(Rb0+data.pos).*cos(theta) );
